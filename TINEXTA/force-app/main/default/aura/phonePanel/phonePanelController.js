...
({
    ...
    // handler for the internal/external call on click
    toggleOutsideCall: function (cmp, event, helper) {
        var outsideCall = !cmp.get('v.outsideCall');
        cmp.set('v.outsideCall', outsideCall);
        if (outsideCall) {
            var toggleMessage = cmp.find("message");
            $A.util.toggleClass(toggleMessage, "toggle");
        }
    ...
    handleSelectCard: function (cmp, event, helper) {
        ...
        if (!selectedRecord.Phone) {
            cmp.set('v.searchResults', []);
            cmp.set('v.message', $A.get("$Label.c.CTI_PhonePanelController_ContactWithoutPhone"));
        } else {
