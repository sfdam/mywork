...
({
    ...
    handleOutgoingCalls : function(cmp) {
        var listener = function(payload) {
            console.log('SV handleOutgoingCalls: ', payload);
                ...
                callback : function() {
                    if (cmp.isValid() && cmp.get('v.presence') != 'Unavailable') {
                        ...
                        var attributes = {
                            ...
                            'title' : '',
                            'account' : payload.recordId,
                            'presence' : cmp.get('v.presence'),
                            'outsideCall' : cmp.get('v.outsideCall')
    ...
    callNumber : function(cmp, number) {
        ...
        //attributes.username = cmp.get("v.username");
        //attributes.password = cmp.get("v.password");
    ...
    callContact : function(cmp, record) {
        if (!record ) {
            throw new Error($A.get("$Label.c.CTI_PhonePanelHelper_CallContactError"));
        };
        var attributes = {
            'state' : 'Dialing',
            'outsideCall' : cmp.get('v.outsideCall'),
    ...
    search : function(cmp, inputValue, onCompletion) {
        ...
        if (inputValue.length < 2) {
            cmp.set('v.message', $A.get("$Label.c.CTI_PhonePanelHelper_SearchCheckInput"));
            return;
        };
        var args = {
            ...
            callback : function(result) {
                if (result.success) {
                    ...
                    throw new Error( $A.get("$Label.c.CTI_PhonePanelHelper_SearchError"));
                }
            }
        };
    ...
    updatePresence : function(cmp, event, helper) {
        ...
        if (newStatus === 'Unavailable') {
            newMessage = $A.get("$Label.c.CTI_PhonePanelHelper_UnavailableState");
