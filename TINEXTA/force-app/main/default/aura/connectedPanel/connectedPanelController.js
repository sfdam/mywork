...
({
    ...
    endCall: function(cmp, event, helper) {
        helper.logCall(cmp, function(response) {
            cmp.getEvent('renderPanel').setParams({
                type : 'c:phonePanel',
                toast : {'type': 'normal', 'message':  $A.get("$Label.c.CTI_ConnectedPalnelController_ToastEndCall")},
    ...
    transferCall: function(cmp, event, helper) {
        cmp.getEvent('renderPanel').setParams({
              toast : {'type': 'normal', 'message': $A.get("$Label.c.CTI_ConnectedPalnelController_ToastTransferCall")+ cmp.get('v.inputValue') +'.'},
        }).fire();
    },
})