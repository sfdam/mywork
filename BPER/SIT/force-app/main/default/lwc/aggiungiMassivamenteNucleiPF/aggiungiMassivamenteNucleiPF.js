...
export default class AggiungiMassivamenteNucleiPF extends NavigationMixin(LightningElement) {
    ...
    loadNDGRecords(isSearch){
        ...
        try{
            ...
            if(selectedMwRows.length > 0 || !this.step1){
                ...
                console.log('@@@ ndg: '+this.searchedNdg);
                console.log('@@@ offset: '+this.ndgOffset);
                console.log('@@@ ndgLimit: '+this.ndgLimit);
                return loadNdgList({
    ...
    handleSelection(event){
        ...
        if(this.modelliDiServizio.split(',').includes(this.selectedModelloDiServizio)){
            ...
            let tempListFull=event.detail.selectedRows;
            let tempList=tempListFull.map(item=>item.Id);
            
            let accList= this.ndgList;
            let pad = this.padri;
            let fig= this.figli;

            let length=accList.length;
            //padre selezionato
            for(let i=0; i<length-1 ; i++){
                if(pad.includes(accList[i].Id) && srId.includes(accList[i].Id) && !this.currentRowsStep2.includes(accList[i].Id)){
                    for (let j=i+1; j<length; j++){
                        if(accList[i].PTF_Nucleo__c===accList[j].PTF_Nucleo__c){
                            tempList.push(accList[j].Id);
                            tempListFull.push(accList[j]);
                        }
                        else{
                            break;
                        }
            ...
            for(let i=0; i<length-1 ; i++){
                if(pad.includes(accList[i].Id) && !srId.includes(accList[i].Id) && this.currentRowsStep2.includes(accList[i].Id)){
                    for (let j=i+1; j<length; j++){
                        if(accList[i].PTF_Nucleo__c===accList[j].PTF_Nucleo__c){
                            let index= tempList.indexOf(accList[j].Id);
                            tempList.splice(index,1);
                            tempListFull.splice(index,1);
                        }
                        else{
                            break;
                        }
            ...
            for(let i=1; i<length ; i++){
                if(fig.includes(accList[i].Id)  && srId.includes(accList[i].Id) && !this.currentRowsStep2.includes(accList[i].Id)){
            ...
            this.selectedRowsStep2=this.selectedRowsStep2.concat(tempList);
            this.currentRowsStep2=this.currentRowsStep2.concat(tempList);
            this.selectedNdgRows=tempListFull;
