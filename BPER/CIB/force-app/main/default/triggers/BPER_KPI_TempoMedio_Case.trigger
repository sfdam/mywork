trigger BPER_KPI_TempoMedio_Case on Case (before update) {
    ...
    if(defaultBH != NULL){
        for(Case caseObj : trigger.new ){
            ...
            if(caseObj.Case_End_Escalation_Date__c != NULL && Trigger.oldMap.get(caseObj.Id).Case_End_Escalation_Date__c != caseObj.Case_End_Escalation_Date__c){
				//For BH method we assign (BH record id, start time field, end time field)
				decimal result = BusinessHours.diff(defaultBH.Id, caseObj.Case_Start_Escalation_Date__c, caseObj.Case_End_Escalation_Date__c );
				...
				caseObj.KPI__c = resultingHours.setScale(1); 
            }  
        }    
    } 
