...
function onChatEventReleasedOutbound(message) {
  ...
  function onChatEventReleased(message) {
    ...
    if (message.attachdata.media_type == "whatsapp") {
        ...
        sfutil.updateUnbluChatEnded(message.attachdata.case_id);
        sfutil.unbluEndConversation(message.attachdata.unblu_chat_id);
    }
/*
...
function onInteractionBlindTransfer(message){
    ...
    if ( ixn?.attachdata?.case_id && ixn?.attachdata?.media_type == 'unblu_chat' && ixn?.attachdata?.unblu_chat_id ){
        ...
        sfutil.closeTab(ixn?.attachdata?.case_id);
    }
}
