...
export default class MultipleAssignment extends LightningElement {
        ...
        try {
            ...
            if(this.selectedNDGs.length > 0){
                ...
                this.getRelatedRecords().then(() =>{ //valerio.salvati
                    ...
                    this.selectedNDGs.forEach(ndg => {
                        ...
                        if(this.cointestazioniCorrelateMap[ndg.Id]){
                            this.cointestazioniCorrelateMap[ndg.Id].forEach(co =>{
                                ...
                                if(co. CRM_JointOwnership__r.ModelloDiServizio__c=='Non Portafogliati'){
                                    if(!blockedNDGMS[ndg.Id]){
                                        blockedNDGMS[ndg.Id] = [];
                                    }
                                    blockedNDGMS[ndg.Id].push(co);
                                }
                    ...
                    console.log('DK blockedNDGMS', JSON.stringify(blockedNDGMS));
                    let messaggio;
                    if(blockedNDG){
                        messaggio = '';
                        console.log('DK selectedNDGMap', selectedNDGMap);
                    ...
                    if(blockedNDGMS){
                        messaggio = '';
                        console.log('DK selectedNDGMap', selectedNDGMap);
                        for (const [key, value] of Object.entries(blockedNDGMS)) {

                            console.log('DK key', key);
                            console.log('DK NaturaGiuridica', selectedNDGMap[key].PTF_NaturaGiuridica__c);
                            let ndgJoin = selectedNDGMap[key].PTF_NaturaGiuridica__c == 'PF' ? blockedNDGMS[key].map(element => element.CRM_JointOwnership__r.CRM_NDG__c).join(', ') : blockedNDGMS[key].map(element => element.CRM_Account__r.CRM_NDG__c).join(', ');
                            messaggio += 'Spostamento ndg ' + selectedNDGMap[key].CRM_NDG__c + ' non consentito perchè gli ndg relazionati (' + ndgJoin + ') hanno portafoglio vuoto. \n';
                        }
    ...
    getRelatedRecords(){
        ...
        if(listaPFId.length > 0 || listaCointestazioniId.length > 0){
            ...
            .then(result => {
                ...
                console.log('cointestazioniCorrelateMap: ', result["cointestazioniCorrelateMap"]);
                console.log('altreCointestazioni: ', result["altreCointestazioni"]);
                console.log('altreCointestazioniMap: ', result["altreCointestazioniMap"]);
    ...
    multipleRecordFilter(recordMap){
        ...
        console.log('pfMap: ', pfMap);
        this.pfCorrelateMap = Object.fromEntries(pfMap);
        console.log('pfCorrelateMap: ', this.pfCorrelateMap);
        ...
        console.log('altreCoMap: ', altreCoMap);
        ...
        console.log('cointestazioniCorrelateMap: ', this.cointestazioniCorrelateMap);
        //return errorFound;
        this.relatedRecords = relatedRecords;
        console.log('relatedRecords: ', this.relatedRecords);
        return false;
    }
