trigger OnCampaignMemberTrigger on CampaignMember (before insert, before update, after insert, after update) {
    ...
    if(trigger.isInsert){
        if(trigger.isBefore){
            ...
            // MS - Aggiunto utente ETL
            if(f.isMC__c || f.isETL__c){
                OnCampaignMemberTriggerHandler.setFields(Trigger.new, Trigger.oldMap, true);
            }
        }
    ...
    if(trigger.isUpdate){
        if(trigger.isBefore){
            ...
            // MS - Aggiunto utente ETL
            if(f.isMC__c || f.isETL__c){
                OnCampaignMemberTriggerHandler.setFields(Trigger.new, Trigger.oldMap, false);
            }
        }
