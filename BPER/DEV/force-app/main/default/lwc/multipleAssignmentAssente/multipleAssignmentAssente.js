...
// MS 12-02-24 aggiunta custom label
import RingFencerError from '@salesforce/label/c.PTF_multipleAssignmentAssente_RingFenceError';
import RingFence from '@salesforce/label/c.multipleAssignmentAssente_CLM_RingFence';
...
const COLUMNS = [
    ...
    {
        label: RingFence,
        fieldName: 'CRM_Specchio_BPER__c',
        type: 'text'
    }
];
...
export default class MultipleAssignmentAssente extends NavigationMixin(LightningElement) {
    ...
    @track searchedNome = '';
    @track searchedNDG = '';
    // MS 19-02-2024 aggiunto campo ring fence
    @track searchedFence;
    @track mwList = [];
    ...
    set_size = 50;
        ...
        // MS 15-11-23 Start
        @track searchedNomeRef = '';
        @track searchedModello = '';
        @track mwListComplete = [];

        @track optionsModello =
        [
            ...
            { label: 'Key Client Imprese', value: 'Key Client Imprese'},
            ...
            { label: 'Business PMI', value: 'Business PMI'},
            { label: 'Couco', value: 'Couco'}
        ]
        // MS 15-11-23 End
    ...
    loadNDGRecords(){
        ...
        try{
            ...
            .then(result => {
                ...
                this.showNDGs = true;
                console.log('showNDGs' ,this.showNDGs);
                console.log('result loadAssenteNdgList' ,result);
        ...
        try{
            ...
            .then(result => {
                ...
                try {
                    ...
                    this.mwList = !result["mwList"] ? [] : this.selectedNDGs[0].CRM_Specchio_BPER__c ? result["mwList"].filter(element =>{return element.PTF_ModelloDiServizio__c.toLowerCase() === 'cesare ponti'}) : result["mwList"];
                    ...
                    // MS 15-11-23 Start
                    this.mwListComplete = this.mwList;
                    // MS 15-11-23 End
            ...
            if(this.selectedNDGs.length > 0){
                ...
                checkCointestazione({
                    ...
                    this.loadMWRecords().then(() =>{
                        console.log('loadMWRecords result ');
                        ...
                        this.showNDGs = false;
                        this.loaded = true;
                        console.log('showNDGs false ',this.showNDGs);
                    });
    ...
    handleAssignNDGs() {
        ...
        try {
            ...
            console.log('CRM_Specchio_BPER__c: ' + this.selectedNDGs[0].CRM_Specchio_BPER__c);

            // MS start 08-02-2024
            for(let i=0; i<mwSelected.length; i++)
            {
                if (this.selectedNDGs[0].CRM_Specchio_BPER__c && mwSelected[i].PTF_ModelloDiServizio__c != 'Cesare Ponti')
                {
                    this.dispatchToast('Warning!',
                    RingFencerError,
                    'warning');
                    throw RingFencerError;
                }
            }
            //return
            // MS end
            ...
            if(mwSelected.length > 0){
                ...
                if(this.superUser){
                    ...
                    if(configurationMap[mapKey]){
                        ...
                        console.log('DK CURRENT CHECK PTF_SegmentoComportamentale__c', this.selectedNDGs[0].PTF_SegmentoComportamentale__c);
                        console.log('DK CURRENT CHECK PTF_ModelloDiServizio__c', mwSelected[0].PTF_ModelloDiServizio__c);
                        if(configurationMap[mapKey].Check_SegComp__c && this.selectedNDGs[0].PTF_SegmentoComportamentale__c.toLowerCase().includes(mwSelected[0].PTF_ModelloDiServizio__c.toLowerCase())){
    ...
    handlePreviousSelected(){
        ...
        this.showNDGs = true;
        console.log('showNDGs true ',this.showNDGs);
    }
    ...
    handleNDGSearch(){
        ...
        for(var i in this.ndgListComplete){
            ...
            // MS 19-02-2024
            if(this.searchedFence != !this.ndgListComplete[i].CRM_Specchio_BPER__c){
    ...
    // MS 15-11-23 Start
    handleMWReset(){
        this.searchedModello = '';
        this.searchedNomeRef = '';
        this.mwList = this.mwListComplete; 
        this.setPagesMW(this.mwListComplete);
    }

    handleMWSearch(){
        this.mwList = [];
        this.pageMW = 1;
        for(var i in this.mwListComplete){
        
            if(Boolean(this.searchedNomeRef)){
            
                if(!this.mwListComplete[i].Referente.toLowerCase().includes(this.searchedNomeRef.toLowerCase()))continue;
            }
            if(Boolean(this.searchedModello)){
            
                if(!this.mwListComplete[i].PTF_ModelloDiServizio__c.toLowerCase().includes(this.searchedModello.toLowerCase()))continue;
            }
            this.mwList.push(this.mwListComplete[i]);
        }
        this.setPagesMW(this.mwList);
    }
    // MS 15-11-23 Start
    ...
    handleFilter(event){
        ...
        else if(event.target.name == 'searchedNomeRef'){
                
            this.searchedNomeRef = event.target.value;
        }
        else if(event.target.name == 'searchedModello'){

            this.searchedModello = event.target.value;
        }
        // MS 19-02-24
        else if(event.target.name == 'searchedFence'){

            this.searchedFence = event.target.value;
        }
    }
}