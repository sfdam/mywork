...
import {TABELLA_ELENCOCAMPAGNE, TABELLA_ELENCOMMDS,TABELLA_ELENCOREF,TABELLA_ELENCOOPP,TABELLA_ELENCOOPPSTEP2,TABELLA_ELENCOREFDAASSEGNARE,JSON_ElegibleRoleMMDS,COLUMNSLISTAREFRENTI,COLUMNSREFERENTI1, optionsStato, optionsMMDS, optionsPriorita, optionsCanale, optionsBisogno, optionsEsito, optionsFase, optionsEsitoOpp, extendCampaignEventMembers, sortBy } from './utils/assegnaAzioniTabHandler';
...
export default class AssegnaAzioniTab extends NavigationMixin(LightningElement) {
    ...
    oppColumnsStep2 = TABELLA_ELENCOOPPSTEP2;
    ...
    @track disabledButtonAssegnaMassivamenteOpp = true;
    @track disabledButtonAssegnaOppStep2 = true;
    @track disabledButtonAssegnaOppStep3 = true;
    @track isAggiungiMassivamenteOPP = false;
    ...
    handleGetOppsMember(){
        ...
        .finally(() =>{
            ...
            oppListData.forEach(opp => {
                //console.log('newOppList.size: ',newOppList.length);
                let exists = newOppList.some(newOpp => newOpp.CRM_Bisogno__c === opp.CRM_Bisogno__c && newOpp.CRM_Canale__c === opp.CRM_Canale__c);
                
                /*console.log('opp.CRM_Bisogno__c: ', opp.CRM_Bisogno__c);
                console.log('newOpp.exist: ', exists);
                console.log('opp.CRM_Canale__c: ', opp.CRM_Canale__c);*/

                if (!exists)
                {
                  newOppList.push({ Id: this.makeId(), CRM_Bisogno__c: opp.CRM_Bisogno__c, CRM_Canale__c: opp.CRM_Canale__c, contattiMdsFamily: 0, contattiMdsPoe: 0, nonPortafogliati: 0, residuale: 0, assente: 0, totale: 0, oppList: []});  
                }
                //console.log('newOppList result', newOppList);
                
            });
            console.log('newOppList.length: ',newOppList.length);
            for (let i = 0; i < newOppList.length; i++) {
                let newOpp = newOppList[i]     
                oppListData.forEach(opp => {

                    if(newOpp.CRM_Bisogno__c === opp.CRM_Bisogno__c && newOpp.CRM_Canale__c === opp.CRM_Canale__c && !opp.hasOwnProperty('CRM_Assegnatario__c') ) {
                        if (opp.PTF_ModelloDiServizio === "Family") {
                            newOpp.contattiMdsFamily++;
                        }
                        else if (opp.PTF_ModelloDiServizio === "POE") {
                            newOpp.contattiMdsPoe++;
                        }
                        else if (opp.PTF_ModelloDiServizio === "Non Portafogliati") {
                            newOpp.nonPortafogliati++;
                        }
                        else if (opp.PTF_ModelloDiServizio === "Residuale") {
                            newOpp.residuale++;
                        }
                        else if (opp.PTF_ModelloDiServizio === "Assente") {
                            newOpp.assente++;
                        }
                        newOpp.Id = i+1
                        newOpp.totale++
                        newOpp.oppList.push(opp);
                    }

                });
            };

            oppListData = newOppList
            // MS 12-12-2023 end
            this.opportunityData = oppListData;
    ...
    handleAssegnaReferenteRequestCMTab(event){
        if(this.handleAssegnaReferenteRequestCMTabClicked == false){
            ...
            this.messaggioIsBatch = this.selectedCMTabRow.length > this.asyncThreshold ? 'Verrai notificato quando l\'assegnazione massiva sarà completata.\n' : '';
    ...
    selectedOpportunityGroups = [];
    selectedOpportunityGroupsId = [];
    handleOppSelection(event){
        try {
            
            this.selectedOpportunityGroups = event.detail.selectedRows;
            this.selectedOpportunityGroupsId = event.detail.selectedRows.map(element => element.Id);
            var allSelectedOppList = [];
            event.detail.selectedRows.forEach(element =>{
                console.log('DK element', JSON.stringify(element.oppList));
                allSelectedOppList.push(...element.oppList);
            })
            console.log('DK allSelectedOppList', JSON.stringify(allSelectedOppList));
            if(event.detail.selectedRows.length > 1){
    
                this.disabledButtonAssegnaMassivamenteOpp = false; 
                ...
                this.disabledButtonAssegnaOpp = true;
                this.disabledButtonAssegnaMassivamenteOpp = true; 
            }
            this.membersOPPTab = JSON.parse(JSON.stringify(allSelectedOppList));
            console.log('DK this.membersOPPTab', JSON.stringify(this.membersOPPTab));
            this.varTab = 'OPPTab';
            this.handleReset();
            this.handleSearch();
            this.setPages(this.filteredMembersOPPTab);
    ...
    handleOppSelectionStep2(event){
        ...
        if(countSelectedOppRows > 0){
            ...
            this.disabledButtonAssegnaOppStep2 = true;
        }
    }

    handleSendRequestOPPTab(event){
        this.counterOPPTab = 0;
        this.OPPTabSelectRowsMapValues = {};
        this.OPPTabSelectRowsMap = {};
        this.varTab ='OPPTab';
        ...
        this.assOppStep3 = false;
        //valerio.salvati
        this.searchedStatoAssegnazioneOPPTab = 'Non assegnato'
        console.log('DK this.membersOPPTab', JSON.stringify(this.membersOPPTab));
        this.handleSearch();
    }
    ...
    handleAssegnaOPPTabRequest(event){
        let selectedOppRows = [];
        if(event.target.name == 'assegnaMassivamenteOpportunities'){
            
            console.log('DK this.membersOPPTab.length', this.membersOPPTab.length);
            selectedOppRows = this.membersOPPTab.filter(record => {return record.CRM_StatoAssegnazione === 'Non assegnato'});
            console.log('DK selectedOppRows.length', selectedOppRows.length);
            this.isAggiungiMassivamenteOPP = true;
        }else{
            let oppMembersTable = this.template.querySelector("[data-item='oppMembersTableStep2']");
            // aggiungo alla lista degli elementi selzionati le righe nelle pagina corrente
            if(oppMembersTable){
            ...
            this.isAggiungiMassivamenteOPP = false;
        }

        this.selectedOPPTabRow = JSON.parse(JSON.stringify(selectedOppRows));
        console.log('SV LIST: ', JSON.stringify(selectedOppRows));
    ...
    handleReferenteOPPTabSelection(event){
        ...
        this.disabledButtonAssegnaOppStep3 = false;
    }
    ...
    handleGetRefrenti(){
        ...
        Object.keys(dataMap).forEach(function(key){
            ...
            serializedResults += '{"Id":"' + idUtente + '","nomeRisorsa":"' + nomeRisorsa + '","numeroContattiCampagne":' + numeroContattiCampagne+',"numeroContattiOpportunita":'+numeroContattiOpportunita;
            ...
            if(allCampaign.size > 0 || allOpp.size > 0){
                let listSize = [allCampaign.size,allOpp.size];
                let maxSize = 0;
    ...
    handleSave(){
        ...
        if(this.handleSaveClicked == false){
            ...
            this.showSpinner = true;
                ...
                let start = new Date().getTime();
                let messaggio = this.tableCMREFTabIdRows.length > this.asyncThreshold ? 'assegnazione in corso...' : undefined;
                reassignItem({evList : null, cmList : this.tableCMREFTabIdRows, oppList : this.tableOPPREFTabIdRows, toUser: selectedRowsRef[0].id, asyncThreshold: this.asyncThreshold})
    ...
    onRowSelectionRefTabStep2C(event){
        ...
        if(this.counterCMREFTab > 0 ||  this.counterOPPREFTab > 0){          
    ...
    handleAvanti(){
        try {
            let tab = this.assCamStep2 ? 'CMTab' : this.assOppStep2 ? 'OPPTab' : this.varTab != '' && this.varTab != undefined ? this.varTab : '';
            ...
            this.template.querySelector("[data-item='tableOPPREFTab']").getSelectedRows();
    ...
    handleReset(){
        let tab = this.assCamStep2 ? 'CMTab' : this.assOppStep2 ? 'OPPTab' : this.varTab != '' && this.varTab != undefined ? this.varTab : '';
        let filterTab = ['CMREFTab', 'OPPREFTab'].includes(tab) ? 'REFTab' : tab;
        if(Boolean(this['searchedStatoAssegnazione' + filterTab]))this['searchedStatoAssegnazione' + filterTab] = filterTab != 'OPPTab' ? 'Non assegnato' : '';//valerio.salvati
    ...
    handleSearch(){
        try {
            ...
            console.log('DK handleSearch filteredList ' + tab + ':', filteredList);
    ...
    handleGetCurrentReferenteData(selectedRows){
        ...
        let arr = (this.assCamStep2 || this.isAggiungiMassivamenteCM) ? this.idReferentiCMTabAssegnate : this.idReferentiOPPTabAssegnate;
    ...
    handleBack(event){
        if(this.assRefStep2){
            ...
            this.assOppStep2 = false;
            this.assOppStep3 = false;
        }else if(this.assOppStep3){
            this.assOppStep3 = false;
            console.log('Dk this.isAggiungiMassivamenteOPP', this.isAggiungiMassivamenteOPP);
            if(this.isAggiungiMassivamenteOPP){
                this.assOppStep1 = true;
                this.assOppStep2 = false;
                this.selectedOpportunityGroupsId = this.selectedOpportunityGroups.map(element => element.Id);
            }else{
                this.assOppStep1 = false;
                this.assOppStep2 = true;
                this.selectedOPPTabRowIds = this.selectedOPPTabRow.map(element => element.Id);
            }
        }
    }
    ...
    onHandleSort( event, refTabVar, tableName) {
        try {
            ...
            let tab = refTabVar != '' && refTabVar != undefined ? refTabVar : this.nLoad <= 1 ? '' : this.assCamStep2 ? 'CMTab' : this.assOppStep2 ? 'OPPTab' : this.varTab != '' && this.varTab != undefined ? this.varTab : '';
            ...
            cloneData.sort( sortBy( sortedBy != 'PTF_AccountUrl' ? sortedBy : 'CRM_NDG', sortDirection === 'asc' ? 1 : -1 ) );
            ...
            console.log('DK '+sortedBy+' '+this.sortDirectionCMREFTab+' ' );
