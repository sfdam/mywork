...
import search from '@salesforce/apex/FAC_CustomLookupController.getSearchResult';

export default class Fac_CustomLookup extends LightningElement {
    @api pReadOnly = false;
    @api required = false;
    ...
    doSearch(){
        let strToSearch = this.strSearch;
            ...
            if(strToSearch.length >= this.minStrSearch && !this.pReadOnly){
                this.isLoading = true;
                this.template.querySelector('.slds-combobox').classList.add('slds-is-open');
                search({ strSearch: strToSearch, fieldAlternativeInLabel: this.fieldAlternativeInLabel, otherFieldToQuery: this.otherFieldToQuery, fieldToLike: this.fieldToLike, sObjectName: this.sObjectName, iconName: this.iconName, condition: this.whereCondition}).then(res => {
