...
export default class NewMicroWalletModal extends NavigationMixin(LightningElement) {
    ...
    getRefsPrivate(valueFiliale, valueModServizio, allValid){
        getGestoriPrincipaliPrivate({idFiliale: valueFiliale, modelloServizio: valueModServizio, ufficioPrivate: this.selectedOption, abi: this.abi, isCheckedPTF: this.isPTF999, isPTFSvil: this.isPTFSvil})
        .then(result => {
            this.getAllGestoriPrincipali = result;
    ...
    handleSave(){
        ...
        if(allValid){
            if(!gestorePrincipale.some(item => gestoreBackup.includes(item))){
                ...
                isPTFSvil : this.isPTFSvil}) // LV CR NEC #70081
                    .then(result => {
