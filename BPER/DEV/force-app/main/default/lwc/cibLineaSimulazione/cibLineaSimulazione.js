...
export default class CibLineaSimulazione extends LightningElement {
    ...
    connectedCallback(){
        ...
        if(this.lineaSimulazione.Rate_Linee__r){
            this.periodiErogazione = this.lineaSimulazione.Rate_Linee__r.filter(element => {return element.Type__c === 'Periodo Erogazione';});
            this.interestStepTassoFisso = this.lineaSimulazione.Rate_Linee__r.filter(element => {return element.Type__c === 'Interest Step Tasso Fisso';});
            this.interestStepCommissioneFirma = this.lineaSimulazione.Rate_Linee__r.filter(element => {return element.Type__c === 'Interest Step Commissione Firma';});
            this.ammortamentoAdHoc = this.lineaSimulazione.Rate_Linee__r.filter(element => {return element.Type__c === 'Piano Ammortamento';});
        }
        ...
        this.lineaSimulazione.periodiErogazione = [...this.periodiErogazione];
        this.lineaSimulazione.interestStepTassoFisso = [...this.interestStepTassoFisso];
        this.lineaSimulazione.interestStepCommissioneFirma = [...this.interestStepCommissioneFirma];
        this.lineaSimulazione.ammortamentoAdHoc = [...this.ammortamentoAdHoc];
    ...
    handleFilter(event){
        ...
        try {
            ...
            this.lineaSimulazione = JSON.parse(JSON.stringify(this.lineaSimulazione));
            let fieldToCheck = event.target.name.split('#')[0];
            let initialValue = this.lineaSimulazione[fieldToCheck];
            let finalValue = event.target.type == 'checkbox' ? event.target.checked :
            event.target.type == 'number' ? Number(event.target.value) : ['true', 'false'].includes(event.target.value) ? /^true$/i.test(event.target.value) : event.target.value;
            
            let changedValue = event.target && fieldToCheck && initialValue != finalValue;
            
            this.lineaSimulazione[fieldToCheck] = finalValue;

            if(fieldToCheck === 'Formatecnica__c'){
                if(this.lineaSimulazione.Formatecnica__c.toLowerCase().includes('fideiussioni')){
                    ...
                    this.lineaSimulazione.Tipo_funding__c = 'No funded';
            ...
            /*if(fieldToCheck === 'Preammortamento__c'){
                this.lineaSimulazione.SAL__c = !Boolean(this.lineaSimulazione[fieldToCheck]);
            }*/
            
            if(fieldToCheck === 'SAL__c' && this.lineaSimulazione.SAL__c === false){
                console.log('DK modifica sal - false');
                this.periodiErogazione = [];
            }
            if(fieldToCheck === 'UpFrontValorePerc__c'){
            ...
            if(fieldToCheck === 'RunningValorePerc__c'){
            ...
            if(fieldToCheck === 'AltroValorePerc__c'){
            ...
            this.extendLinea();
            if(fieldToCheck === 'Intereststep__c'){
            ...
            if(fieldToCheck === 'IntereststepCF__c'){
            ...
            if((fieldToCheck === 'Tipo_di_ammortamento__c' || fieldToCheck === 'Formatecnica__c' || fieldToCheck === 'Durata__c' || fieldToCheck === 'Periodicita_rata__c') && this.lineaSimulazione.Formatecnica__c.includes('MLT')){
                if(this.lineaSimulazione.Tipo_di_ammortamento__c !== 'ad_hoc'){
                    this.ammortamentoAdHoc = [];
                }else if(this.ammortamentoAdHoc.length === 0){
            ...
            if((fieldToCheck === 'Numero_periodi_di_erogazione__c') && changedValue){
            ...
            if(fieldToCheck === 'Durata__c' || fieldToCheck === 'Periodicita_rata__c'){
                ...
                if(!this.lineaSimulazione.Numerodirate__c){
                    ...
                    let inputField = fieldToCheck === 'Durata__c' ? this.template.querySelectorAll('[name="Periodicita_rata__c"]') : this.template.querySelectorAll('[name="Durata__c"]');
                    console.log('DK CJECK DURATAPERIOD', inputField.length);
    ...
    returnRecord(){
        try {
            ...
            console.log('DK isInputValid.periodiErogazione', this.periodiErogazione);
            console.log('DK isInputValid.interestStepTassoFisso', this.interestStepTassoFisso);
            console.log('DK isInputValid.interestStepCommissioneFirma', this.interestStepCommissioneFirma);
            console.log('DK isInputValid.ammortamentoAdHoc', this.ammortamentoAdHoc);
            for(var i = 0; i < inputFields.length; i++){
                
                this.lineaSimulazione[inputFields[i].name.split('#')[0]] = inputFields[i].style && inputFields[i].style.display == 'none' ? null : inputFields[i].type == 'checkbox' ? inputFields[i].checked :
    ...
    isInputValid() {
        ...
        try {
            ...
            let isValid = true;
            // let inputFields = this.template.querySelectorAll('.validateLinea');
            var lineaSimulazioneCopy = {...this.lineaSimulazione};
            this.lineaSimulazione = {...this.lineaSimulazione};
            // console.log('DK isInputValid.inputFields', inputFields);
            let visibleFields = ['Simulazione__c', 'Number__c', 'Rate_Linee__r'];
            this.template.querySelectorAll('.validateLinea').forEach(inputField => {
    ...
    fieldVisibilityMap = {
        ...
        'Formatecnica__c': {
            'MLT-Chirografario': ['Cap__c','Floor__c','CMUannuale__c'],
            'MLT-Ipotecario Residenziale': ['Cap__c','Floor__c','CMUannuale__c'],
            'MLT-Ipotecario Commerciale': ['Cap__c','Floor__c','CMUannuale__c'],
            'Linea Term Loan': ['Cap__c','Floor__c','CMUannuale__c'],
            'MLT-Linea RCF': ['Utilizzo__c','CMUannuale__c'],
    ...
    get optionsTipoTassoDelta(){
        return this.loaded && this.lineaSimulazione.Formatecnica__c && (this.lineaSimulazione.Formatecnica__c.toLowerCase().includes('mlt') || this.lineaSimulazione.Formatecnica__c.toLowerCase().includes('loan')) ? this.optionsTipoTasso.filter(element =>{return element.value !== 'COMMISSIONE_DI_FIRMA'}) : this.optionsTipoTasso.filter(element =>{return element.value === 'COMMISSIONE_DI_FIRMA'});
    }

    get optionsTipoFundingDelta(){
        return this.loaded && this.lineaSimulazione.Formatecnica__c && this.lineaSimulazione.Formatecnica__c.toLowerCase().includes('fideiussioni') ? this.optionsTipoFunding.filter(element =>{return element.value === 'No funded'}) : this.optionsTipoFunding.filter(element =>{return element.value !== 'No funded'});
    }
