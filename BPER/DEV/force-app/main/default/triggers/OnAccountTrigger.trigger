...
trigger OnAccountTrigger on Account (before update,  after update) {
    ...
    if(OnIdentityDocumentTriggerHandler.skipAccountTrigger) return;//alessandro di nardo @ten
    ...
    if(trigger.isUpdate){
        if(trigger.isBefore){
            ...
            //OnAccountTriggerHandler.aggiornaPortafogliazione(Trigger.new,Trigger.oldMap);
			// MS 08-11-2023 - fine
        }
