trigger OnCRMIngaggioRetiEsterneTrigger on CRM_IngaggioRetiEsterne__c (after insert, after update) {
    if(trigger.isInsert){
        if(trigger.isBefore){
        ...
        if(trigger.isAfter){
            ...
            OnCRMIngaggioRetiEsterneTriggerHandler.trackHistory(Trigger.oldMap, Trigger.newMap);
        }        
    }
