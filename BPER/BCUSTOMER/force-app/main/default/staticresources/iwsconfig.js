...
function loadPureRequestConfig() {
    ...
    var params = {
        customInteractionAttributes: ["CRM_CustomerAuthenticationType", "esigenza_SF","ndgId","secureflow","caseidreal","accountid","accountname","casenumber", "abi","callbacknumbers","dialercampaignId","dialercontactId","dialercontactlistId","dialerinteractionid","scriptid","voicemail","Id","CRM_NumeroTentativi__c",
                                     "sfdc_id","case_id","task_id","contact_id","EVENT_ID","context.prechat_id","context.livechat_id","service_channel_id","work_item_id","id","psr_id","ani","motivo_chat","codice_fiscale","media_type","name","unblu_chat_id"],
        settings: {
            sso: false,
    ...
    var url = `https://apps.${softphoneSettings.PEF_GC_ENVIRONMENT}/crm/softphoneGenericCRM/index.html?request_configuration=true&crm_domain=${window.location.origin}&full_PEF=true&language=it`;
    logsf.info("url : ", url);
    var config = {
        context: window,
        layoutType: "frame",
