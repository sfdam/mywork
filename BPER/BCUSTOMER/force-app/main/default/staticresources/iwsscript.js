...
async function onActivatePureEmbeddableSessionFullPEF(message) {
    ...
    try {
        ...
        if (!softphone_listener_initialized) {
            ...
            sfutil.addTabClosedListener();
            sfutil.addPresenceListener();
            await sfutil.buildPresenceStatuses();
            window["gcUser"] = await pClient.usersApi.getUsersMe();
            unbluPersonId = await sfutil.unbluPersonSearch();
            sfutil.getGenesysAgentId();
            softphone_listener_initialized = true;
        }
    } catch (e) {
        logsf.info("onActivatePureEmbeddableSessionFullPEF error : ", e);
...
async function onChatEventRingingInbound(message) {
    ...
    if (message.attachdata.media_type != "unblu_chat") {
...
async function onChatEventEstablishedInbound(message) {
    ...
    } else if (message.attachdata.case_id && message.attachdata.media_type == 'unblu_chat' && message.attachdata.unblu_chat_id) {
        var add = await sfutil.unbluAddParticipant(message.attachdata.unblu_chat_id);
        await sfutil.unbluSetAssigneePerson(message.attachdata.unblu_chat_id);
        if (add) {
            sfutil.updateUnbluChatAccepted(message.attachdata.case_id,message.InteractionID);
...
async function onChatEventReleased(message) {
    logsf.info("onChatEventReleasedOutbound , message : ", message);
    ...
    } else if (message.attachdata.media_type == "unblu_chat") {
        if (mapTransferred[message.InteractionID]) {
            delete mapTransferred[message.InteractionID];
            sfutil.closeTab(message?.attachdata?.case_id);
            return;
        }
        await sfutil.updateUnbluChatEnded(message.attachdata.case_id);
        await sfutil.unbluEndConversation(message.attachdata.unblu_chat_id);
        sfutil.closeTab(message?.attachdata?.case_id);
    }
    /*
...
// Open Event
//==============================================
function onOpenEventRingingInbound(message) {
    logsf.info("onOpenEventRingingInbound , message : ", message);
    onChatEventRingingInbound(message);
}
function onOpenEventEstablishedInbound(message) {
    logsf.info("onOpenEventEstablishedInbound , message : ", message);
    onChatEventEstablishedInbound(message);
}

function onOpenEventReleasedInbound(message) {
    logsf.info("onOpenEventReleasedInbound , message : ", message);
    onChatEventReleased(message)
}
function onOpenEventMarkDoneInbound(message) {
    logsf.info("onOpenEventMarkDoneInbound , message : ", message);
    
}


function onInteractionBlindTransfer(message) {
    logsf.info("onInteractionBlindTransfer :", message);
    const id = message.SourceMsg;
    const ixn = iwscore.mapInteractions[id];
    if (ixn?.attachdata?.case_id && ixn?.attachdata?.media_type == 'unblu_chat' && ixn?.attachdata?.unblu_chat_id) {
        mapTransferred[message.SourceMsg] = ixn;
        sfutil.unbluRequeueConversation(ixn?.attachdata?.unblu_chat_id);
    }
}
