({
	getTodayTask : function(component, event, helper) {
		var isDirezioneFD = component.get("v.isDirezioneFD");
		console.log('@@@ isDirezioneFD ' , isDirezioneFD);
		var action = component.get("c.getTodayActivityUser_FD");
		action.setParams({
		...
		action.setCallback(this, (response) =>{
			if(response.getState() == "SUCCESS"){
				...
				if(risposta.success){
					console.log('@@@ risposta header attività oggi ' , risposta.data);
	...
	getFilteredData : function(component, event, helper, parametri){
		var action = component.get("c.getTodayActivityUser_FD_Filtered");
		action.setParams({
